...
//------------------------------------------------------------------------------
    ...
    //下方定义了回调函数的通用类型，即可用Callback进行声明，其返回类型是void，形式参数列表是两个Address,其实也是void
...
//------------------------------------------------------------------------------
    ...
    {
        return *static_cast<W*>(pw);//static_cast是强制转换，将输入pw转换成<class W>的类型
    }
...
//------------------------------------------------------------------------------
    ...
    public:
        ...
        {}//小控件需要传入它的位置和性状，名称以及该控件的回调函数

        virtual void move(int dx,int dy) { hide(); pw->position(loc.x+=dx, loc.y+=dy); show(); }
        virtual void hide() { pw->hide(); }
        virtual void show() { pw->show(); }
        virtual void attach(Window&) = 0;
        // virtual void hilight() {pw->color(FL_RED);}
        // virtual void dehilight() {pw->color(FL_BACKGROUND_COLOR);}
        Point loc;
        int width;
        int height;
        string label;
